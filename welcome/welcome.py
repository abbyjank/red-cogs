...
default_settings = {
    ...
    "WHISPER_GREETING": [default_greeting],
    "ON": False,
    "LEAVE_ON": False,
    "LEAVE_CHANNEL": None,
    "GROUPED": False,
...
class Welcome(Events, commands.Cog):
    ...
    async def welcome_settings(self, ctx: commands.Context) -> None:
        ...
        setting_names = {
            ...
            "WHISPER_GREETING": _("Random Whisper/DM Greeting "),
        ...
        if ctx.channel.permissions_for(ctx.me).embed_links:
            ...
            for attr, name in setting_names.items():
                if attr in ["GREETING", "GOODBYE", "WHISPER_GREETING"]:
                    embed.add_field(
                        name=name,
                        value="\n".join(g for g in guild_settings.get(attr, []))[:1024],
                        inline=False,
                    )
                    continue
                if attr in ["CHANNEL", "LEAVE_CHANNEL"]:
                    chan = guild.get_channel(guild_settings.get(attr))
                    if chan is not None:
                    ...
                    continue
                if attr == "BOTS_ROLE":
                    role = guild.get_role(guild_settings.get("BOTS_ROLE"))
                    if role is not None:
                    ...
                    msg += f"**{name}**: {guild_settings.get(attr)}\n"
            ...
            msg = "```\n"
            for attr, name in setting_names.items():
                msg += name + str(guild_settings.get(attr)) + "\n"
            msg += "```"
            await ctx.send(msg)
    ...
    @whisper.command(name="add")
    async def whisper_add(self, ctx: commands.Context, *, format_msg: str) -> None:
        """
        Adds a whisper/DM greeting message format for the guild to be chosen at random
        ...
        await ctx.send(_("Whisper/DM greeting message added for the guild."))

    @whisper.command(name="list", aliases=["edit", "delete", "del"])
    async def whisper_list(self, ctx: commands.Context, raw: bool = False) -> None:
        """
        Lists the whisper/DM greeting messages of this guild and allows editing the settings.

        - `[raw=False]` Whether to show the raw text. This can be toggled afterwards.
        """
        guild_settings = await self.config.guild(ctx.guild).WHISPER_GREETING()
        if not guild_settings:
            await ctx.send(_("You have no saved whisper/DM greetings."))
            return
        source = WelcomePages(guild_settings)
        menu = BaseMenu(source, self, raw=raw, event_type=EventType.whisper_greeting)
        log.debug({c.custom_id for c in menu.children})
        await menu.start(ctx)
