...
class Events:
    ...
    async def send_member_join(
        ...
        whisper_msgs = await self.config.guild(guild).WHISPER_GREETING()

        if not msgs and not whisper_msgs:
            return
        ...
        if not await self.config.guild(guild).GROUPED():
            if await self.config.guild(guild).WHISPER():
                try:
                    chosen_whisper_msgs = whisper_msgs or msgs
                    if chosen_whisper_msgs:
                        whisper_msg = rand_choice(chosen_whisper_msgs)
                        if is_embed:
                            em = await self.make_embed(member, guild, whisper_msg, True)
                            if await self.config.guild(guild).EMBED_DATA.mention():
                                await member.send(member.mention, embed=em)  # type: ignore
                            else:
                                await member.send(embed=em)  # type: ignore
                        else:
                            await member.send(await self.convert_parms(member, guild, whisper_msg, False))  # type: ignore
                except discord.errors.Forbidden:
                    log.info(
    ...
    async def send_testing_msg(
        ...
        whisper_choices = guild_settings.get("WHISPER_GREETING") or choices or [default_greeting]
        ...
        if not bot and (guild_settings["WHISPER"] or whisper):
            ...
            if is_embed:
                em = await self.make_embed(member, guild, whisper_rand_msg, is_welcome)
                await ctx.author.send(embed=em, delete_after=60)
            else:
                await ctx.author.send(
                    await self.convert_parms(member, guild, whisper_rand_msg, is_welcome),
                    delete_after=60,
                    allowed_mentions=allowed_mentions,
                )
