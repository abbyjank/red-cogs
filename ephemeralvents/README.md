...
- **Lifecycle & Background Automation (`tasks.loop`)**:
  ...
  - **Archival or Deletion**:
    - `archive`: Hides channel from `@everyone` (`view_channel=False`), grants read-only access to moderators, optionally moves to an archive category, and posts a moderator deletion button.
